...
def VollCellSeg(image: np.ndarray, 
    ...
    if noise_model is None and star_model is not None and  roi_model is not None and cellpose_model is not None:
        ...
        if 'T' not in axes:   
            ...
            vollcellseg = CellPoseWater(cellpose_base, masks, Sizedsmart_seeds, Big_roi_image, min_size_mask, max_size,nms_threshold)
        if 'T' in axes:
            ...
            for time in range(image_membrane.shape[0]):
                ...
                for z in range(Big_roi_image.shape[0]):
                    Big_roi_image[z,:,:] = roi_image
                vollcellseg_time = CellPoseWater(cellpose_base_time, masks_time, Sizedsmart_seeds[time,:,:,:], Big_roi_image, min_size_mask, max_size,nms_threshold)
